...
```python
plt.bar(x, height)
plt.bar(x, height, width=0.8, bottom=None, color=None, label=None)

参数	作用	                                        默认值	实用示例
x	    x 轴数据（分类标签，如名称、类别 ID）	            -	     x = ['A产品', 'B产品', 'C产品']
height	柱子的高度（核心数值，与 x 一一对应）	            -	     height = [50, 80, 65]
width	柱子的宽度（0~1 之间，值越大越宽）	            0.8	     width=0.6（更窄的柱子）
bottom	柱子的底部基准（默认从 0 开始，可实现堆叠柱状图）	None	 bottom=[10, 20, 15]（柱子从指定高度开始）
color	柱子颜色（支持单个颜色或颜色列表）	            None	 color='red' 或 color=['r','g','b']
label	柱子的标签（用于图例说明）	                    None	 label='2024年销量'
```
...
### **Q6. In `plt.subplot(2,1,3)`, what does `2` mean?**
